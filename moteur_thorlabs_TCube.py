...
import moteur
from thorlabs_kinesis import TCube_DC_Servo_Fonction as TCube
...
class MoteurThorlabs_TCube(moteur.Moteur):
    """
    Classe moteur thorlabs pour TCUBE
    """
    def __init__(self, serial: str, control_type: str, moteur_type: str, lang: str = "FR"):
        """
        Initialisation de la classe moteur_thorlabs pour TCUBE.
        """
        super().__init__()
        ...
        self.TCube = TCube.TCubeDCServoFonction()
        self.TCube.__init___(self.serial)
        self.TCube.TLI_BuildDeviceList()

        # Ouverture de la connection
        if self.TCube.open() == 0:
            # identification du controleur en cours (clignotement)
            self.TCube.identify()

            self.TCube.start_polling(milliseconds)
            self.TCube.clear_message_queue()
            sleep(1.0)
            ...
            self.TCube.request_vel_params()
            [vel, acc] = self.TCube.get_vel_params()
            ...
            self.rt = Periodic(0.1, self.update_position) # it auto-starts, no need of rt.start()
    ...
    def velocity(self):
        ...
        self.TCube.set_vel_params(vel, acc)
    ...
    def fast_pos_step(self):
        ...
        """
        if self.TCube.check_connection():
            # position actuelle
            pos_act = int(self.TCube.get_position())
    ...
    def move_to(self, target: int, lock_motor: bool = True):
        ...
        if self.TCube.check_connection():
            ...
            self.TCube.set_move_absolute_position(target)
            sleep(0.1)

            # Envoi de l'ordre de déplacement
            self.TCube.move_absolute()
            sleep(0.1)
    ...
    def is_moving(self):
        ...
        print("ismoving")
        while not moved:
            if self.TCube.message_queue_size() != 0: # read the Serial Meassage Queue
                [msg_type, msg_id, msg_data] = self.TCube.get_next_message()
        ...
        if self.TCube.check_connection():
            ...
            if err == 0:
                while True:
                    ...
                    current_pos = int(self.TCube.get_position())
