...
# Ajout par NF. Contient les paramêtres des moteurs
MotorLimitSettings = namedtuple("MotorLimitSettings",
                                ["minPosition",
                                 "maxPosition",
                                 "minVelocity",
                                 "maxVelocity",
...
motor_encoder_lib = {
    ...
    "PRM1-Z8": MotorEncoderSettings(1919.64, 42941.66, 14.66),
    "MTS25-Z8": MotorEncoderSettings(34304, 767367.49, 261.93),
    "MTS50-Z8": MotorEncoderSettings(34304, 767367.49, 261.93),
}

# Ajout par NF ( minPos, maxPos, minVel, maxVel, minAcc, maxAcc )
motor_limit_lib = {
        "PRM1-Z8": MotorLimitSettings(0, 691070.4, 0, 1073499, 0, 366),
        "MTS25-Z8": MotorLimitSettings(0, 857600, 0, 1841681.976, 0, 1178.685),
        "MTS50-Z8": MotorLimitSettings(0, 1715200, 0, 1841681.976, 0, 1178.685),
        }

# Renvoi une valeur limite du moteur
def get_motor_limit_param(motor_type: str, dimension: str="minPosition") -> float:
    motor_limit = motor_limit_lib.get(motor_type, None)

    if motor_limit is None:
        raise ValueError("Can't find given motor encoder.")

    limit = getattr(motor_limit, dimension, None)
